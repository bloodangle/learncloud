...
public class LessonForm {
    ...
    public void setOpenclassdate() {
        SimpleDateFormat time = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        System.out.print(time.format(new Date()));
        this.openclassdate = time.format(new Date());
    }

    public Lesson convertToLesson()
    {
        Lesson lesson =new LessonFormConvert().convert(this);
        return lesson;
    }

    private class LessonFormConvert implements FormConvert<LessonForm, Lesson>{

        @Override
        public Lesson convert(LessonForm lessonForm){
            Lesson lesson = new Lesson();
            BeanUtils.copyProperties(lessonForm,lesson);
            return lesson;
        }
    }
}
